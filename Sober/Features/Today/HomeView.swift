...
struct HomeView: View {
    ...
    /// The day count the *tree* is drawn at: the streak plus whatever growth a
    /// slip banked. The counter above it keeps showing the honest streak.
    ...
    private var checkInControl: some View {
        if !checkedInToday && daysMissed > 1 {
            ...
            // bonsai on the day they told it they used.
            HStack(spacing: 12) {
    ...
    private func presentPostOnboardingPaywallIfNeeded() {
        ...
        postOnboardingPaywallTask = Task { @MainActor in
            ...
            // the delay window. Leaving it set retries on the next Home arrival.
            // MainTabView drops a pitch while a review sheet is up, so the flag
            // is only spent once the request can actually be honoured.
            guard !showGrowth, !showReviewPrompt,
                  !ReviewPromptCoordinator.shared.isPresentingSheet else { return }
            AppGroup.defaults.set(false, forKey: AppGroup.postOnboardingPaywallKey)
