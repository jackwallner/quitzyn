...
enum AppGroup {
    ...
    /// Set once the pre-craving-release check-in rows have been marked as
    /// tended. `wasLogged` shipped with a default of false, so without this
    /// every day an earlier build recorded came back reading as "the app
    /// assumed this".
