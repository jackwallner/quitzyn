...
final class SubscriptionService: NSObject {
    ...
    func syncConversionAttributes() {
        ...
        //
        // The empty check has to come after the merge. It used to sit on the
        // event counts alone, which meant an install with no onboarding events
        // would never have sent the paywall keys either.
...
/// RevenueCat app inside the same project and cannot touch App Store customers,
/// revenue or charts.
///
...
enum RevenueCatProbe {
    ...
    static let testStoreKey = "test_UeazyQDOgujIcyaxDqIUgBztEBV"

    static var appUserID: String {
        ProcessInfo.processInfo.environment["RC_PROBE_USER"] ?? "funnel-probe-quitzyn"
    }
