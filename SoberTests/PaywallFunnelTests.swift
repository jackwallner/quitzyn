...
struct PaywallFunnelTests {
    ...
    @Test func countsPitchesPerSurfaceAndInTotal() throws {
        try withFreshSuite {
            ConversionDiagnostics.recordPitchView(impressionID: "quitzyn_bloom_tab")
            ConversionDiagnostics.recordPitchView(impressionID: "quitzyn_bloom_tab")
            ConversionDiagnostics.recordPitchView(impressionID: "quitzyn_trial_sheet")
            let a = ConversionDiagnostics.subscriberAttributes
            #expect(a["pitch_views_total"] == "3")
            #expect(a["pitch_views_bloom_tab"] == "2")
            #expect(a["pitch_last"] == "trial_sheet")
        }
    }
    ...
    @Test func conversionFreezesTheSurfaceAndCountAtTheMomentOfSale() throws {
        try withFreshSuite {
            ...
            ConversionDiagnostics.recordPitchView(impressionID: "quitzyn_bloom_tab")
            let a = ConversionDiagnostics.subscriberAttributes
            #expect(a["converted_surface"] == "trial_sheet")
    ...
    @Test func attributeKeysStayInsideRevenueCatsLimit() throws {
        try withFreshSuite {
            ...
            ConversionDiagnostics.recordPitchView(impressionID: "quitzyn_\(long)")
