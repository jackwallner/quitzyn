...
set -euo pipefail
cd "$(dirname "$0")/.."
# Default to the live App Store ID from .astro-app.json (falls back to 6784788496).
APP_ID="${ASTRO_APP_ID:-$(python3 -c "import json;print(json.load(open('scripts/.astro-app.json'))['appId'])" 2>/dev/null || echo 6784788496)}"
